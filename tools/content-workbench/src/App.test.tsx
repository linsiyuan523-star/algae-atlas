...
function createServerApi(): ServerApi {
  return {
    ...
    getPendingStatus: vi.fn(async () => ({
      ok: false,
      action: "pending-status",
      code: "QUEUE_NOT_INITIALIZED",
      message: "Content queue has not been initialized",
    })),
    ...
    getSyncStatus: vi.fn(async () => ({
      ok: false,
      action: "sync-status",
      code: "SYNC_TRANSACTION_NOT_FOUND",
      message: "No synchronization transaction was found",
    })),
    syncPendingNow: vi.fn(async () => ({
      ok: false,
      action: "sync-pending",
      code: "NO_PENDING_CONTENT",
      message: "No pending content changes",
    })),
